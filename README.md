...
- **Stable entities**: ecobee issues a new `acknowledgeRef` each time it raises the same reminder again (e.g. the next filter change), so the `binary_sensor` object_id is derived from the alert's `alertNumber` instead (a fixed numeric code ecobee documents, e.g. `3140` = HVAC maintenance, `3135` = UV lamp) -- not the ref, and not the free-text body either. That keeps one entity per distinct reminder that just flips `on`/`off` across recurrences, instead of spawning a new entity each time -- easier to build a persistent automation/notification against. `acknowledgeRef` and the full alert `text` are still published as attributes for reference.
- **Entity naming**: known maintenance-reminder `alertNumber`s are mapped to the same short names ecobee's own app uses (e.g. "HVAC Maintenance", "Furnace Filter", "UV Lamp") instead of the full sentence ecobee sends as `text`. Unrecognized alert numbers fall back to the raw text.
- **Entities exist before they fire**: every *enabled* reminder configured on the thermostat (Settings -> Reminders in the ecobee app) gets an entity right away, defaulting to `off`, not just ones currently due -- so you can build a notification/automation against it ahead of time instead of waiting for the entity to first appear. It flips `on` once ecobee actually raises the alert. Disabled reminders (e.g. a UV lamp reminder turned off) get no entity until re-enabled.
